...
![](images/gerrymandering_nass.png){width=85%}
...
BOTTOM LINE: A real, peer-reviewed claim, summarized with a mean, looked convincing; the same data summarized with a median told a different and more honest story. The mean is easy to explain and it is what most people reach for — and it is exactly the wrong summary when a few extreme cases can dominate it. So is it true that female hurricanes are more deadly on average?  Well, it depends on what measure you think is best -- a decision that requires an argument.
...
The mean is used most of the time for continuous variables, but as we have just seen, it can be VERY sensitive to outliers and lead misleading characterizations. The median (50th percentile) is usually better for describing what is "typical," but the downside is that it can be difficult to explain to general audiences. 
...
So the *average* district was essentially tied, but the *median* district favored Trump by about 3 points.  So how should we decide what to say about the "typical" district?
...
Now California, where the tools were used in the other direction: the safest Democratic districts were *cracked* -- CA-02 went from Harris +45 to +25, CA-04 from +31 to +15 -- and those voters were used to flip CA-01, CA-41 and CA-48 from Trump districts into Harris districts.  Spreading your own voters more thinly makes the median district less Democratic in the hope of winning more seats.  
...
*Quick Exercise* Redo the competitiveness categories using `dem_margin_2026map` to create a new variable called `competitive_2026map`. Did redistricting make districts more or less competitive?

```{r}
# INSERT CODE HERE
```

*Quick Exercise* Can you compare the two measures of competitiveness?  Let's see how they compare under the new and old maps.

```{r}
# INSERT CODE HERE
``` 

So which measure reveals the effects of redistricting?  The mean? The median?
...
$$\text{Percentage Point Change: } X_{new}\% - X_{old}\%$$
...
$$\text{Percent Change: } 100 \times \frac{X_{new} - X_{old}}{X_{old}}\%$$
...
# New R Functions Introduced in Session 5

Functions introduced in Sessions 2–4 are intentionally omitted.
...
| `median()` | Calculates the median, the middle value after sorting. |
